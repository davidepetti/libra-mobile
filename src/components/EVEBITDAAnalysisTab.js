...
const EVEBITDAAnalysisTab = ({ files, holdingPeriod }) => {
    ...
    const loadAndProcessFiles = async () => {
      try {
        ...
        if (!files || files.length === 0) {
          throw new Error('No EV/EBITDA files provided');
        }
          ...
          Papa.parse(fileContent, {
            ...
            complete: (results) => {
              if (results.data && results.data.length > 0) {
                ...
                const processedData = processCSVData(results.data, 'ev_ebitda');
        ...
        const dataWithProfit = calculateAnnualizedProfit(allData, holdingPeriod, 'ev_ebitda');

        // Calculate statistics
        const evEbitdaValues = dataWithProfit.map(item => item.ev_ebitda).filter(val => !isNaN(val));
        const stats = {
          current: evEbitdaValues[evEbitdaValues.length - 1] || 0,
          mean: evEbitdaValues.reduce((sum, val) => sum + val, 0) / evEbitdaValues.length,
          median: evEbitdaValues.sort((a, b) => a - b)[Math.floor(evEbitdaValues.length / 2)],
          min: Math.min(...evEbitdaValues),
          max: Math.max(...evEbitdaValues)
        };

        // Calculate percentile data
        const percentiles = [10, 25, 50, 75, 90];
        const percentileValues = percentiles.map(p => {
          const index = Math.floor(evEbitdaValues.length * (p / 100));
          return evEbitdaValues[index];
        });
        ...
        const percentileData = percentiles.map((percentile, index) => {
          ...
          const filteredData = dataWithProfit.filter(item => 
            item.ev_ebitda >= value * 0.95 && item.ev_ebitda <= value * 1.05
          );
        ...
        console.error('Error loading EV/EBITDA data:', error);
        setError(`Error loading EV/EBITDA data: ${error.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <View style={styles.loaderContainer}>
        ...
        <Text>Loading EV/EBITDA data...</Text>
      </View>
    );
  }
  ...
  if (!data || data.length === 0) {
    return (
      <View style={styles.emptyContainer}>
        <Text>No EV/EBITDA data available</Text>
      </View>
    );
  }
  ...
  const trendData = {
    labels: data
      .filter(item => !isNaN(item.ev_ebitda) && isFinite(item.ev_ebitda))
  ...
  const scatterData = {
    labels: data
      .filter(item => 
        !isNaN(item.ev_ebitda) && 
        isFinite(item.ev_ebitda) && 
  ...
  return (
    <ScrollView style={styles.container}>
      <Card style={styles.card}>
        <Card.Content>
          <Title>Enterprise Value to EBITDA (EV/EBITDA) Analysis</Title>
      ...
      <Card style={styles.card}>
        <Card.Content>
          <Title>EV/EBITDA Ratio Over Time</Title>
          <LineChart
            data={trendData}
      ...
      <Card style={styles.card}>
        <Card.Content>
          <Title>EV/EBITDA Ratio vs Annualized Return</Title>
          <LineChart
            data={scatterData}
            width={screenWidth - 40}
            height={220}
            ...
            style={styles.chart}
          />
          <Text style={styles.chartNote}>Note: Each point represents EV/EBITDA ratio and its corresponding {holdingPeriod}-year annualized return</Text>
        </Card.Content>
      </Card>
        ...
        <Card.Content>
          ...
          <DataTable>
            <DataTable.Header>
              ...
              <DataTable.Title>EV/EBITDA Value</DataTable.Title>
...
export default EVEBITDAAnalysisTab;
