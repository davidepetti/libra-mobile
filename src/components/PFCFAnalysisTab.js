...
const PFCFAnalysisTab = ({ files, holdingPeriod }) => {
    ...
    const loadAndProcessFiles = async () => {
      try {
        ...
        if (!files || files.length === 0) {
          throw new Error('No P/FCF files provided');
        }
          ...
          Papa.parse(fileContent, {
            ...
            complete: (results) => {
              if (results.data && results.data.length > 0) {
                ...
                const processedData = processCSVData(results.data, 'pfcf_ratio');
        ...
        const dataWithProfit = calculateAnnualizedProfit(allData, holdingPeriod, 'pfcf_ratio');

        // Calculate statistics
        const pfcfValues = dataWithProfit.map(item => item.pfcf_ratio).filter(val => !isNaN(val));
        const stats = {
          current: pfcfValues[pfcfValues.length - 1] || 0,
          mean: pfcfValues.reduce((sum, val) => sum + val, 0) / pfcfValues.length,
          median: pfcfValues.sort((a, b) => a - b)[Math.floor(pfcfValues.length / 2)],
          min: Math.min(...pfcfValues),
          max: Math.max(...pfcfValues)
        };

        // Calculate percentile data
        const percentiles = [10, 25, 50, 75, 90];
        const percentileValues = percentiles.map(p => {
          const index = Math.floor(pfcfValues.length * (p / 100));
          return pfcfValues[index];
        });
        ...
        const percentileData = percentiles.map((percentile, index) => {
          ...
          const filteredData = dataWithProfit.filter(item => 
            item.pfcf_ratio >= value * 0.95 && item.pfcf_ratio <= value * 1.05
          );
        ...
        console.error('Error loading P/FCF data:', error);
        setError(`Error loading P/FCF data: ${error.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!data || data.length === 0) {
    return (
      <View style={styles.emptyContainer}>
        <Text>No P/FCF data available</Text>
      </View>
    );
  }
  ...
  const trendData = {
    labels: data
      .filter(item => !isNaN(item.pfcf_ratio) && isFinite(item.pfcf_ratio))
  ...
  const scatterData = {
    labels: data
      .filter(item => 
        !isNaN(item.pfcf_ratio) && 
        isFinite(item.pfcf_ratio) && 
  ...
  return (
    <ScrollView style={styles.container}>
      <Card style={styles.card}>
        <Card.Content>
          <Title>Price to Free Cash Flow (P/FCF) Analysis</Title>
      ...
      <Card style={styles.card}>
        <Card.Content>
          <Title>P/FCF Ratio Over Time</Title>
          <LineChart
            data={trendData}
      ...
      <Card style={styles.card}>
        <Card.Content>
          <Title>P/FCF Ratio vs Annualized Return</Title>
          <LineChart
            data={scatterData}
            width={screenWidth - 40}
            height={220}
            ...
            style={styles.chart}
          />
          <Text style={styles.chartNote}>Note: Each point represents P/FCF ratio and its corresponding {holdingPeriod}-year annualized return</Text>
        </Card.Content>
      </Card>
        ...
        <Card.Content>
          ...
          <DataTable>
            <DataTable.Header>
              ...
              <DataTable.Title>P/FCF Value</DataTable.Title>
...
export default PFCFAnalysisTab;
