...
const PBAnalysisTab = ({ files, holdingPeriod }) => {
    ...
    const loadAndProcessFiles = async () => {
      try {
        ...
        if (!files || files.length === 0) {
          throw new Error('No P/B files provided');
        }
          ...
          Papa.parse(fileContent, {
            ...
            complete: (results) => {
              if (results.data && results.data.length > 0) {
                ...
                const processedData = processCSVData(results.data, 'pb_ratio');
        ...
        const dataWithProfit = calculateAnnualizedProfit(allData, holdingPeriod, 'pb_ratio');

        // Calculate statistics
        const pbValues = dataWithProfit.map(item => item.pb_ratio).filter(val => !isNaN(val));
        const stats = {
          current: pbValues[pbValues.length - 1] || 0,
          mean: pbValues.reduce((sum, val) => sum + val, 0) / pbValues.length,
          median: pbValues.sort((a, b) => a - b)[Math.floor(pbValues.length / 2)],
        ...
        const percentileValues = percentiles.map(p => {
          const index = Math.floor(pbValues.length * (p / 100));
          return pbValues[index];
        });
        ...
        const percentileData = percentiles.map((percentile, index) => {
          ...
          const filteredData = dataWithProfit.filter(item => 
            item.pb_ratio >= value * 0.95 && item.pb_ratio <= value * 1.05
          );
        ...
        console.error('Error loading P/B data:', error);
        setError(`Error loading P/B data: ${error.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!data || data.length === 0) {
    return (
      <View style={styles.emptyContainer}>
        <Text>No P/B data available</Text>
      </View>
    );
  }
  ...
  const trendData = {
    labels: data
      .filter(item => !isNaN(item.pb_ratio) && isFinite(item.pb_ratio))
  ...
  return (
    <ScrollView style={styles.container}>
      <Card style={styles.card}>
        <Card.Content>
          <Title>Price to Book (P/B) Analysis</Title>
      ...
      <Card style={styles.card}>
        <Card.Content>
          <Title>P/B Ratio Over Time</Title>
          <LineChart
            data={trendData}
      ...
      <Card style={styles.card}>
        <Card.Content>
          <Title>P/B Ratio vs Annualized Return</Title>
          <LineChart
            data={scatterData}
            width={screenWidth - 40}
            height={220}
            ...
            style={styles.chart}
          />
          <Text style={styles.chartNote}>Note: Each point represents P/B ratio and its corresponding {holdingPeriod}-year annualized return</Text>
        </Card.Content>
      </Card>
        ...
        <Card.Content>
          ...
          <DataTable>
            <DataTable.Header>
              ...
              <DataTable.Title>P/B Value</DataTable.Title>
