...
const PEAnalysisTab = ({ files, holdingPeriod }) => {
    ...
    const loadAndProcessFiles = async () => {
      try {
        ...
        if (!files || files.length === 0) {
          throw new Error('No P/E files provided');
        }
          ...
          Papa.parse(fileContent, {
            ...
            complete: (results) => {
              if (results.data && results.data.length > 0) {
                ...
                const processedData = processCSVData(results.data, 'pe_ratio');
        ...
        const dataWithProfit = calculateAnnualizedProfit(allData, holdingPeriod, 'pe_ratio');

        // Calculate statistics
        const peValues = dataWithProfit.map(item => item.pe_ratio).filter(val => !isNaN(val));
        const stats = {
          current: peValues[peValues.length - 1] || 0,
          mean: peValues.reduce((sum, val) => sum + val, 0) / peValues.length,
          median: peValues.sort((a, b) => a - b)[Math.floor(peValues.length / 2)],
        ...
        const percentileValues = percentiles.map(p => {
          const index = Math.floor(peValues.length * (p / 100));
          return peValues[index];
        });
        ...
        const percentileData = percentiles.map((percentile, index) => {
          ...
          const filteredData = dataWithProfit.filter(item => 
            item.pe_ratio >= value * 0.95 && item.pe_ratio <= value * 1.05
          );
        ...
        console.error('Error loading P/E data:', error);
        setError(`Error loading P/E data: ${error.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!data || data.length === 0) {
    return (
      <View style={styles.emptyContainer}>
        <Text>No P/E data available</Text>
      </View>
    );
  }
  ...
  const trendData = {
    labels: data
      .filter(item => !isNaN(item.pe_ratio) && isFinite(item.pe_ratio))
  ...
  return (
    <ScrollView style={styles.container}>
      <Card style={styles.card}>
        <Card.Content>
          <Title>Price to Earnings (P/E) Analysis</Title>
      ...
      <Card style={styles.card}>
        <Card.Content>
          <Title>P/E Ratio Over Time</Title>
          <LineChart
            data={trendData}
      ...
      <Card style={styles.card}>
        <Card.Content>
          <Title>P/E Ratio vs Annualized Return</Title>
          <LineChart
            data={scatterData}
            width={screenWidth - 40}
            height={220}
            ...
            style={styles.chart}
          />
          <Text style={styles.chartNote}>Note: Each point represents P/E ratio and its corresponding {holdingPeriod}-year annualized return</Text>
        </Card.Content>
      </Card>
        ...
        <Card.Content>
          ...
          <DataTable>
            <DataTable.Header>
              ...
              <DataTable.Title>P/E Value</DataTable.Title>
